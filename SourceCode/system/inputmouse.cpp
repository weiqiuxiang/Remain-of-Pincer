...
void CInputMouse::Update(void)
{
	...
	memcpy(&zdiMouseState_bak, &m_zdiMouseState, sizeof(zdiMouseState_bak));       //今のマウスの状態を取得
	...
	if (SUCCEEDED(m_pDIDevice->GetDeviceState(sizeof(DIMOUSESTATE), &m_zdiMouseState)))
	{
		for (int nCnMouse = 0; nCnMouse < NUM_MOUSEKEY; nCnMouse++)
		{
			// キープレス情報を保存
			//キートリガーリリース情報の作成
			m_aMouseStateTrigger[nCnMouse] = (zdiMouseState_bak.rgbButtons[nCnMouse] ^ m_zdiMouseState.rgbButtons[nCnMouse]) & m_zdiMouseState.rgbButtons[nCnMouse];                          //g_aMouseStateは前,aMouseStateは今
